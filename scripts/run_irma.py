...
def _parse_args():
    ...
    p.add_argument("--rad-day0",        type=float, default=None,
                   help="Fractional calendar day-of-year (1-based, UT) at nstep=0 "
                        ...
                        "Overrides the auto-computed value from START_TIME.  "
                        "Set to enable SW radiation; omit for LW-only mode.")
    p.add_argument("--rad-iyear",       type=int, default=None,
                   help="Calendar year for RRTMG orbital geometry (e.g. 1999 to "
                        "match gSAM tracesini iyear=1999).  Overrides the default "
                        "which is START_TIME.year.  Ignored when --rad-day0 is not set.")
...
def main():
    ...
    _day0_frac_auto = (START_TIME.timetuple().tm_yday
                       + (START_TIME.hour * 3600
                          + START_TIME.minute * 60
                          + START_TIME.second) / 86400.0)

    # --rad-day0 overrides the auto-computed value; absent → auto from START_TIME.
    # rad_day0=None disables SW entirely (LW-only).
    if args.rad == "rrtmg":
        _rad_day0 = args.rad_day0 if args.rad_day0 is not None else _day0_frac_auto
    else:
        _rad_day0 = None

    # --rad-iyear overrides the default (START_TIME.year); matches gSAM iyear=1999.
