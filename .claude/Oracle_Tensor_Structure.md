...
### Reconstructing global IRMA-box tensors from rank files (python sketch)
...
def read_header(f):
    ...
    names = [f.read(8).decode() for _ in range(nfields)]
    return dict(i_min_gl=i_min_gl, j_min_gl=j_min_gl,
                it_off=it_off, jt_off=jt_off,
                i1=i1, i2=i2, j1=j1, j2=j2,
                has_overlap=(i1 <= i2 and j1 <= j2))
# global box size
NI_GL = i_max_gl - i_min_gl + 1   # columns in IRMA box
NJ_GL = j_max_gl - j_min_gl + 1
# for each record: global slice indices are
#   ig = (it_off + i1..i2) - i_min_gl   (0-based within the IRMA box)
#   jg = (jt_off + j1..j2) - j_min_gl
# record shape: (ni, nj, nzm, 7), Fortran order -> reshape with order='F'
rec_bytes_hdr = 8                                    # nstep + stage_id
rec_bytes_data = (i2-i1+1)*(j2-j1+1)*NZM*NFIELDS*4
```

Canonical reassembly output per (nstep, stage_id): `np.ndarray` of shape
`(NI_GL, NJ_GL, NZM, 7)` float32, initialised with NaN then filled rank-by-rank.

## What to compare against jsam

1. **Global scalars first**: join `globals.csv` on `(nstep, stage_id)` against jsam's
   equivalent reductions — catches gross divergence early.
2. **Then IRMA-box tensors**: load rank files, reassemble, compare per field per stage.
3. **Stages 17+ only** are valid for QC/QV/QI comparison — earlier stages contain
   the previous step's values (see note above).
4. Stage 12 (`adamsB`) shows an intentional W spike before pressure — do not flag
   as divergence.
